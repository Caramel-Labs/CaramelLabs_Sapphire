...
export default function Chat() {
	...
	useEffect(() => {
		const fetchMessages = async () => {
			try {
				const response = await fetch(`http://localhost:4000/api/chat/64aeb7d2f0c64c1a2f3bcd5a`, {
					method: "GET",
					headers: {
	...
	return (
		<div className="flex-1 p-2 sm:p-6 justify-between flex flex-col h-screen relative bg-white">
			{/* Header */}
			<ChatHeader />
			...
			<div
				id="messages"
				className="flex flex-col space-y-1 p-3 pt-0 overflow-y-auto pb-96 bg-white mb-20"
			>
				{/* Chat messages */}
				{messages &&
