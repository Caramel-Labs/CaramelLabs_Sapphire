...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      switch (activeTab) {
        case 'Travel':
          ...
          requestOptions = {
            ...
            body: JSON.stringify({ interestIds: ['66d1f7537c2ac1b3047e5fd6'] }),
          };
          break;
        case 'Stay':
          endpoint = 'hotels';
          requestOptions = {
            ...
            body: JSON.stringify({ facilityName: 'Co-working spaces' }),
          };
          break;
        case 'Experience':
      ...
      try {
        const response = await fetch(`http://localhost:4000/api/${endpoint}`, requestOptions);
        const fetchedData = await response.json();
        setData((prevData) => ({ ...prevData, [activeTab]: fetchedData })); // Store data separately for each tab
        console.log('Fetched data:', fetchedData);

        // Mark this tab as fetched
        setFetchedTabs((prevFetched) => ({ ...prevFetched, [activeTab]: true }));
      } catch (error) {
  ...
  return (
    <div className="bg-white min-h-screen">
      <Image src="/images/headerImg.png" alt="Sigiriya Rock Fortress" width={360} height={290} />
      <div className="relative">
        <ListingsTab activeTab={activeTab} setActiveTab={setActiveTab} />
      </div>
