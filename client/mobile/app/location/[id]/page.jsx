...
export default function Location(){
    ...
    useEffect(() => {
      const fetchLocationData = async () => {
        try {
          const response = await fetch(`http://localhost:4000/api/places/66d40c19d490bb3a58a34c7d`, {
            method: 'GET',
            headers: {
      ...
      <>
        {place && hotels && (
          <div className="max-w-2xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden ">
            ...
            />
            <div className="p-6">
              <h2 className="text-xl font-semibold mb-2 text-gray-600 ">{place.name}</h2>
              <div className="flex items-center mb-3">
                <span className="text-black text-xs ">8th wonder of the world</span>
            ...
            <p className="text-black mb-4 mt-6 ml-4 mr-4 text-xs">{place?.description}</p>
            <TimeCard timeFromColombo={place.timefromColombo} name={place.name} openHours = {place.openHours}/>
            <ContactCard telephone={place.telephone} email={place.email} web={place.web}/>
