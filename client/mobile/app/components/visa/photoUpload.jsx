...
export default function PhotoUpload() {
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      reader.onload = function (e) {
        ...
        localStorage.setItem('passportSizedPhoto', dataURL);
        setPreviewImage(dataURL);
        console.log('Passport Sized photo saved to localStorage.');
      };
  ...
  const handleRemoveImage = () => {
    localStorage.removeItem('passportSizedPhoto');
    setPreviewImage(null);
    setImageName("");
    console.log('Passport Sized photo removed from localStorage.');
  };

  useEffect(() => {
    const savedImage = localStorage.getItem('passportSizedPhoto');
    const savedImageName = localStorage.getItem('passportSizedPhotoName');
    if (savedImage) {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white">
      ...
      <p className="text-gray-500 text-xs text-center mt-4 mb-8">
        Upload a clear Passport sized photograph of you.
      </p>
      ...
      <button
        className="w-full mt-[180px] py-2 px-4 bg-sapphire text-white rounded-lg text-xs hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 h-[48px]"
